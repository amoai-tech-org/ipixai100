...
- `agents__unsafe_dev_only` and `selfManagedAgents` are dev-only aliases of each other. **Not production-safe.** See [`a2ui-renderer.md`](../a2ui-renderer.md) for A2UI SPA patterns.
...
- `v1 → v2` migration renames: `useCopilotAction` → `useFrontendTool` + `useHumanInTheLoop`; `imageUploadsEnabled` → `attachments`. See [`upgrade/upgrade.md`](../upgrade/upgrade.md).
