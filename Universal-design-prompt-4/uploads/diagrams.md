...
Companion to `00-review-and-conventions.md`, `SCR-32-planner-workspace.md`, `SCR-33-planner-dashboard.md`, `SCR-34-planner-instance-settings.md`. Validate any edits at https://mermaid.live before committing.

---

## 1. Screen hierarchy

Where the 3 new screens sit relative to existing, reused screens.
...
flowchart TD
    ...
    Workspace --> Kanban["Kanban view (reskin of SCR-30 CRM Pipeline)"]
...
**UI mapping:**
- `Draft` → SCR-32 shows the `EmptyState` ("Select a workflow template").
- `Planned` / `Active` → SCR-32 renders populated Timeline/Kanban/Calendar/List.
- `Blocked` → the relevant phase/task bar shows the gate badge; `ApprovalCard` full variant appears in the drawer.
- `AtRisk` → amber `--warning` border on the affected bar/card, surfaced in SCR-33's "At Risk" stat card and SCR-32's IntelligencePanel.
- `Completed` / `Archived` → read-only rendering, no drag handles, no edit controls (same visual treatment as the "read-only role" state already specified in SCR-32's states table).
