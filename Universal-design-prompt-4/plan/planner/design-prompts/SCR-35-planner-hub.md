...
>
> Layout: `PageHeader` ("Planner", subtitle = count + "N need attention" where attention is **derived**), `FilterBar` (entity type `shoot|campaign|crm_deal` · instance status), search, then instance cards (4:3 cover from **linked entity asset or muted placeholder** — `instances` has no cover column; title via naming precedence in `supabase-reference.md`; entity-type badge icon+text; instance `StatusChip`; optional **derived** progress). Same anatomy as a Shoot card.
>
