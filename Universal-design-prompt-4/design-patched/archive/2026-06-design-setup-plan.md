> **⚠️ ARCHIVED — 2026-06-28. Superseded by `plan.md` + `DESIGN.md` + `21-component-dependencies.md` + `prompts/`.**
> This was the pre-implementation setup/governance plan. Its status claims are stale (tokens.css now exists, 12 shadcn primitives installed, Command Center prototype built, readiness no longer 61/100). Kept for reference only — the component-metadata spec format (§3C), the prompt library (§8E), the upload manifest, and the risk register (§12) are not duplicated elsewhere. Do not treat any status/score/checklist here as current.
