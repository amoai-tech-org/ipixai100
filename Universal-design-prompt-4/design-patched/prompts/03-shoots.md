...
> **v3 "Zeely Editorial" overrides** (supersede any v2 styling below): pure white `#FFFFFF` / grey `#FAFAFA` surfaces — **no beige**; **Inter** UI font, Geist Mono numbers only; **black** primary buttons (incl. `+ New shoot`) — **orange is retired** (read any "orange" below as black); selected-card accent = charcoal (not orange); ApprovalCard = **white card + amber hairline + amber dot**, **Approve button black**; active nav = calm thin-grey; 20px card + image radii. ShootCards are **cover-image-first** (4:3 editorial fashion photography of female models in apparel) from `app/design/images` (never stock/illustration/office/glamour). This screen carries the **global AI chat dock** at the base of the center workspace — context-aware greeting, quick chips, streaming status, black send; never "How can I help?".
...
Active nav item: Shoots (Camera icon, `var(--nav-item-active-text)` charcoal on a calm thin-grey `--nav-item-active` fill).
...
**CTA:**
`[+ New shoot]` — `<Button variant="default" size="sm">` full-width, **black**. Opens `/app/shoots/new`.

**Panel tabs** (sticky bottom):
`Details | Shot List | Location | Assets` — `<Tabs>`. Active tab charcoal underline + text.
...
- Center workspace: no cards. Agent greeting: `No shoots planned yet. I can plan your first shoot for Nike based on the campaign calendar.` + `[Plan first shoot]` chip (primary **black** Button)
...
- "Approve & save" = `<Button variant="default" size="sm">` **black**
...
2. The selected card has a 2px **charcoal** left border accent — this ties to the right panel content.
...
5. "New shoot" button appears in two places: workspace header (always visible) and right panel bottom (contextual). Both are identical primary **black** buttons.
