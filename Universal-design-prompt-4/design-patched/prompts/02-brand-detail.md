...
> **v3 "Zeely Editorial" overrides** (supersede any v2 styling below): pure white `#FFFFFF` / grey `#FAFAFA` surfaces — **no beige**; **Inter** UI font, Geist Mono numbers only; **black** primary buttons — **orange is retired** (read any "orange" below as black); ApprovalCard = **white card + amber hairline + amber dot** (no `#FFFBF0` fill), **Approve button black**; active nav = calm thin-grey (not orange); tabs = charcoal underline; 20px card + image radii. Brand hero + before/after diffs use **editorial fashion photography of female models in apparel** from `app/design/images` (never stock/illustration/office/glamour). This screen carries the **global AI chat dock** at the base of the center workspace — context-aware greeting, quick chips, streaming status, black send; never "How can I help?".
...
Active nav item: Brands (Layers icon, `var(--nav-item-active-text)` charcoal on a calm thin-grey `--nav-item-active` fill).
...
- Background: `var(--approval-bg)` = **white card** (amber hairline + amber dot, no fill)
...
- Approve: `<Button variant="default" size="sm">` — **black**
...
`Overview | Assets | Approvals | History` — `<Tabs>` from `ui/tabs.tsx`. Active: charcoal underline + text.
...
- Right panel: DNA section replaced with: Lucide `Zap` icon (24px muted) + `Run Brand Intelligence` subtitle + `[Analyse brand]` primary button (**black**)
...
- Right panel DNA section: animated progress bar (indeterminate, **charcoal**) + `Analysing… 31 of 47 pages` `var(--font-size-xs)` `var(--color-text-muted)` Geist Mono
