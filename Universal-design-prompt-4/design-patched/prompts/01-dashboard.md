...
> **v3 "Zeely Editorial" overrides** (supersede any v2 styling below): pure white `#FFFFFF` / grey `#FAFAFA` surfaces — **no beige**; **Inter** UI font, Geist Mono numbers only; **black** primary buttons (`--color-action`), white + hairline secondaries — **orange is retired** (read any "orange" below as black); ApprovalCard = **white card + amber hairline + amber dot** (no `#FFFBF0` fill), **Approve button black**; active nav = calm thin-grey (not orange); 20px card + image radii. Content objects lead with **editorial fashion photography of female models in apparel** from `app/design/images` (never stock/illustration/office/glamour). This screen carries the **global AI chat dock** at the base of the center workspace — context-aware greeting, quick chips, streaming status, black send; never "How can I help?".
...
- **Workspace** — flex-1. Background `var(--color-bg-page)` (#FFFFFF, pure white). Full height, vertically scrollable if content overflows.
- **IntelligencePanel** — white bg (`var(--intel-bg)`), 1px left border (`var(--intel-border)`). Width: 320px. Never dark mode.

Page background: `var(--color-bg-page)` (#FFFFFF, pure white — no beige).
...
- Brand switcher: show active brand logo/initial (Nike → N) with a small charcoal dot indicator. Badge count `[3]` shown as `var(--color-accent)` pill if approvals pending.
...
- `● Nike  [3]` — charcoal dot, brand name, approval badge
...
- Mic icon (Lucide `Mic`) left, Send icon (Lucide `Send`) right — both `var(--color-text-muted)`, charcoal on hover `var(--color-accent)`
...
Header: `Approvals` label left, badge `[3]` charcoal pill right.

Stack of 3 ApprovalCards (HITL pattern):

Each card:
- Border: 1px `var(--approval-border)` (#F3B93C) hairline
- Background: `var(--approval-bg)` = **white** (amber hairline + amber dot, no fill)
- Radius: 0.625rem
- Padding: 12px
...
- Two buttons: `[Approve]` `[Edit]`
  - Approve: `<Button variant="default" size="sm">` — **black** bg `var(--color-action)`, white text
...
- Center workspace: AI greeting changes to: `Welcome! Let's set up your first brand. I can crawl your website and build your Brand DNA in minutes.` + `[Set up brand]` button (primary, **black**).
...
- Same ApprovalCard style: **white card + amber hairline + amber dot**
...
6. ApprovalCard buttons: Approve = **black** (primary). Edit = outline. Reject = ghost (destructive-light).
...
- Use Inter for all UI text (system fallback: -apple-system, sans-serif)
- Use Geist Mono for numeric data (DNA scores, badge counts); Inter for everything else
