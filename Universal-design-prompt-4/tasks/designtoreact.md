...
Use this for **every** `.dc.html` page in `Universal-design-prompt-4/Pages/` — Command Center, Brand List/Detail, Shoots List/Detail, Shoot Wizard, Assets, Campaigns, Matching, Channel Preview, Analytics, Campaign Performance, Onboarding, CRM (SCR-26–31), and Planner (SCR-32–35).
...
**Worked examples for this template:** [`Shoots List.v2.image-first.dc.html`](../Pages/Shoots%20List.v2.image-first.dc.html) → [IPI-273 / DESIGN-055](https://linear.app/amo100/issue/IPI-273/design-055-shoots-list-react-parity-workspace) and [`Shoot Detail.v2.image-first.dc.html`](../Pages/Shoot%20Detail.v2.image-first.dc.html) → [IPI-337 / DESIGN-054b](https://linear.app/amo100/issue/IPI-337/design-054b-shoot-detail-remaining-tab-parity-6-tabs). Use their current per-screen task docs ([`screens/SCR-04-shoots-list.md`](screens/SCR-04-shoots-list.md), [`screens/SCR-05-shoot-detail.md`](screens/SCR-05-shoot-detail.md)) as the reference shape for every section below — the older `PLAN/<screen>-dc-conversion.md` convention referenced in earlier versions of this doc was superseded by the `tasks/screens/SCR-NN-*.md` convention (see `screens/README.md`).
...
- **Model Booking (SCR-21 Booking Wizard, SCR-22 Booking Detail)** — **standalone screens and routes**, per the engineering override in `Universal-design-prompt-4/docs/models/02-engineering-reference.md` (D1–D9). Do **not** implement these as `?flow=booking` variants of Shoot Wizard/Shoot Detail — that earlier plan was tried and reversed. They are greenfield routes (`/app/matching/talent/[id]/book`, `/app/bookings/[id]`); the DC prototypes reuse Shoot Wizard/Detail markup only as an authoring shortcut. Needs a new `booking` Mastra agent (does not exist yet) and the `requested→quoted→approved→confirmed` status FSM. Shoot Detail itself only gains an inline booking accordion on the crew row — no new tabs.
- **Casting Review (SCR-09 Matching, Talent tab)** — Matching gets a 4th tab (Talent) with a Casting/Grid/List mode switch and a swipe-style review deck; feeds a Shortlist drawer that deep-links to the Booking Wizard above. Spec: `Universal-design-prompt-4/Pages/SCR-09-Matching-Talent.dc.html`.
...
Plans, worktrees, and migrations drift in this repo — see `design-audit.md`'s "Documentation structure & prior-audit integrity" section for real, current examples of docs going stale mid-build in this project. When two sources disagree, resolve in this priority order — don't just pick whichever is easiest to read:
...
4. Latest implementation audit — ⚠️ `tasks/design-docs/shoot/docs/*` referenced in earlier versions of this doc does not exist; use `progress-tracker.md` (repo-root-relative: `Universal-design-prompt-4/progress-tracker.md`) or run a fresh one
```
...
Copy this section list into `Universal-design-prompt-4/tasks/screens/SCR-NN-<slug>.md` (the current convention — see `screens/SCR-TEMPLATE.md` and `screens/README.md`; the older `tasks/design-docs/<feature>/<screen>-dc-conversion.md` + `dc-to-react-plan-template.md` convention referenced in earlier versions of this doc has been superseded) for the screen you're converting.

### 1. Target

```text
HTML source:   Universal-design-prompt-4/Pages/<Screen>.v2.image-first.dc.html
...
| Booking Wizard (SCR-21) | `/app/matching/talent/[id]/book` — **live** (see `progress-tracker.md`) | — | IPI-311 |
| Booking Detail (SCR-22) | `/app/bookings/[id]` — **live** (see `progress-tracker.md`) | — | IPI-312 |
| CRM Companies/Contacts/Pipeline (SCR-26–30) | `/app/crm/*` — **live** (see `progress-tracker.md`) | — | IPI-363/364/395 |
| CRM Deal Detail (SCR-31) | `/app/crm/pipeline/[id]` — 404-gate only, real UI pending Claude Design comps | — | IPI-373 (design), IPI-396 (React) |
| **Planner Workspace (SCR-32)** | `/app/planner/[instanceId]` — **no route yet, greenfield.** Backend (`app/src/lib/planner/engine.ts`) already built + tested; this is UI-only. Not a plain page port — see `planner/planner-react-onboarding.md` first. | — | **IPI-478** |
| **Planner Dashboard (SCR-33)** | `/app/planner/dashboard` — no route yet, greenfield | — | **IPI-479** |
| **Planner Instance Settings (SCR-34)** | `/app/planner/[instanceId]/settings` — no route yet, greenfield | — | **IPI-479** |
| **Planner Hub (SCR-35)** | `/app/planner` — no route yet, greenfield | — | **IPI-526** |

**Planner is not a plain page port** — read `Universal-design-prompt-4/planner/planner-react-onboarding.md` (decision hierarchy, frozen contracts, state matrix) and `Universal-design-prompt-4/planner/high-impact-verification.md` (re-verified reuse/keyboard/mobile/ApprovalCard findings) before starting §5 Reuse Audit on any Planner screen.
