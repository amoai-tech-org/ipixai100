...
**Refresh (IPI-1147 · SPEC — Remove Unexpected Supabase Privileged Function Access and Prove Tenant-Safe RPCs, 2026-09-04):** the register previously enumerated 33 functions but omitted `v2_shoot_owned_by_brand` ([IPI-1110 · SPEC — Sign Cloudinary Uploads for the Trusted Organization](https://linear.app/amo100/issue/IPI-1110)), which the live advisor already counted — so the live advisor count was 34, not 33. This refresh adds `v2_shoot_owned_by_brand` as row 34 (register now matches the live 34). Separately, IPI-1147 revokes direct `PUBLIC`/`anon`/`authenticated` EXECUTE on `talent.log_booking_status_change()` (a trigger function, not a client RPC). That function was **never** part of the advisor's `authenticated_security_definer_function_executable` set (the advisor does not count trigger functions), so the advisor count remains **34** after the revoke. Net register arithmetic: 33 (register) → 34 (live, +`v2_shoot_owned_by_brand`) → 34 (post-revoke, unchanged — trigger was never counted).
